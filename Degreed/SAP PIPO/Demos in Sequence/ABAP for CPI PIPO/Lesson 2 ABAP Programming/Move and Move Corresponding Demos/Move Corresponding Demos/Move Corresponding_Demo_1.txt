* In the below example move corresponding is NOT used
REPORT Z.
DATA: BEGIN OF ADDRESS,
        FIRSTNAME(20) TYPE C VALUE 'Fred',
        SURNAME(20)   TYPE C VALUE 'Flintstone',
        INITIALS(4)   TYPE C VALUE 'FF',
        STREET(20)    TYPE C VALUE 'Cave Avenue',
        NUMBER        TYPE I VALUE '11',
        POSTCODE(5)   TYPE N VALUE '98765',
        CITY(20)      TYPE C VALUE 'Bedrock',
      END OF ADDRESS.

DATA: BEGIN OF NAME,
        SURNAME(20)   TYPE C,
        FIRSTNAME(20) TYPE C VALUE 'Vandana',
        INITIALS(4)   TYPE C,
        TITLE(10)     TYPE C VALUE 'Mister',
      END OF NAME.

MOVE ADDRESS-FIRSTNAME TO NAME-FIRSTNAME.
MOVE ADDRESS-SURNAME   TO NAME-SURNAME.
MOVE ADDRESS-INITIALS  TO NAME-INITIALS.

WRITE:/  NAME-SURNAME.
WRITE:/ NAME-FIRSTNAME.
WRITE:/ NAME-INITIALS.